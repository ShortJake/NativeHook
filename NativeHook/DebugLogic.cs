...
namespace NativeHook
{
    ...
    internal class DebugLogic : MissionLogic
    {
        ...
        internal unsafe static ulong FindAddressInMemory<T>(ulong baseAdr, ulong range, T value, bool searchBackwards) where T : unmanaged
        {
            if (searchBackwards) baseAdr -= range;
        ...
        internal unsafe static long FindOffsetInMemory<T>(ulong baseAdr, ulong range, T value, bool searchBackwards) where T : unmanaged
        {
            ulong addr = FindAddressInMemory<T>(baseAdr, range, value, searchBackwards);
            if (addr == 0x0) return 0x0;
            if (addr > baseAdr) return Convert.ToInt64(addr - baseAdr);
            else return -Convert.ToInt64(baseAdr - addr); 
        }
    }
#endif
}
